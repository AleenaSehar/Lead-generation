...
The target architecture runs email sequences, scoring updates, provider synchronization, and webhook processing as background jobs. Currently sequence and CRM processing are invoked manually through authenticated APIs. Their durable attempts, idempotency keys, leases, and audit events are designed so an automatic worker can adopt them later. Jobs must remain idempotent, retryable, observable, and safe to resume.
...
4. **Provider independence:** Business logic does not depend directly on one external vendor.
