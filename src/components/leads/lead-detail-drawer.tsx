...
export function LeadDetailDrawer({ leadId, canAddNote, canManageWorkflows, onClose, onChanged }: { leadId: string; canAddNote: boolean; canManageWorkflows: boolean; onClose: () => void; onChanged: () => void }) {
  ...
  const [aiInsightFeedback, setAiInsightFeedback] = useState("");
  ...
  async function generateInsight() {
    setBusy(true); setError(""); setAiInsightFeedback("");
    try { await request(`/api/leads/${leadId}/ai-insight`, { method: "POST" }); setAiInsightFeedback("AI insight generated."); await load(); onChanged(); }
    catch (requestError) { setError(requestError instanceof Error ? requestError.message : "Unable to generate AI insight."); }
    finally { setBusy(false); }
  }
  ...
  return <div className="lead-drawer-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
    <aside className="lead-drawer" role="dialog" aria-modal="true" aria-labelledby="lead-drawer-title">
      ...
      {loading && !lead ? <div className="drawer-state">Loading lead history…</div> : error && !lead ? <div className="drawer-state error">{error}</div> : lead && <div className="lead-drawer-body">
        ...
        {canAddNote && <section className="ai-insight-card"><div className="ai-insight-heading"><div><h3>AI lead insight</h3><p>{lead.aiInsight && lead.aiInsightGeneratedAt ? `Generated ${new Intl.DateTimeFormat(undefined, { month: "short", day: "numeric", hour: "numeric", minute: "2-digit" }).format(new Date(lead.aiInsightGeneratedAt))}` : "Qualitative, LLM-based fit assessment"}</p></div><button className="secondary-button" disabled={busy} onClick={() => void generateInsight()}>{busy ? "Generating…" : lead.aiInsight ? "Regenerate" : "Generate insight"}</button></div>{lead.aiInsight ? <div className="ai-insight-body"><div className="ai-insight-score"><strong>{lead.aiInsight.fitScore}</strong><small>AI fit score</small></div><div><p>{lead.aiInsight.summary}</p><ul>{lead.aiInsight.reasons.map((reason, index) => <li key={index}>{reason}</li>)}</ul><p className="ai-insight-next"><b>Next action</b> {lead.aiInsight.nextAction}</p></div></div> : <p className="drawer-muted">Not yet generated. This is a probabilistic AI opinion shown alongside, not instead of, the rule-based score above.</p>}{aiInsightFeedback && <div className="mock-email-success">{aiInsightFeedback}</div>}</section>}
